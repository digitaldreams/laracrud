...
class ViewCrud extends LaraCrud
{
    ...
    public function generateIndexPanel($table = '')
    {
        ...
        $panelmodalTemp = str_replace("@@folderName@@", strtolower($this->getModelName($table)), $panelmodalTemp);
    ...
    protected function tabPanel($tableName)
    {
        ...
        $headerHtml = '';
        $tableName  = !empty($tableName) ? $tableName : $this->mainTable;
        $modelName  = strtolower($this->getModelName($tableName));
        $bodyHtml   = '@foreach($records as $record)'."\n".'<tr>'."\n";
...
//@@properties@@
    ...
    protected function makePartialForm($table)
    {
        ...
        if (!file_exists($pathToSave.'/form.blade.php')) {
    ...
    public function makePartialModal($table)
    {
        ...
        if (!file_exists($pathToSave.'/modal_form.blade.php')) {

            $formContent = $this->generateModal($table);
            $this->saveFile($pathToSave.'/modal_form.blade.php', $formContent);
        }
    }
    ...
    public function getCheckBoxContent($column, $required)
    {
        ...
        $selectTmp       = '';
        if ($required) {
            $selectTmp = '{{old("'.$column['name'].'",$model->'.$column['name'].')==1?"checked":""}}';
        }
